...
router.post("/admin/verify/:paymentId", isAdmin, async (req, res) => {
  ...
  try {
    ...
    if (!payments.length)
      return res.redirect("/admin/daftarPeserta?error=Payment+tidak+ditemukan.");
    ...
    res.redirect(`/admin/daftarPeserta?success=Pembayaran+diverifikasi,+token+${token}+digenerate.`);
  } catch (err) {
...
router.post("/admin/reject/:paymentId", isAdmin, async (req, res) => {
  const { paymentId } = req.params;
  try {
    await db.query("UPDATE payments SET status = 'DITOLAK' WHERE id = ?", [paymentId]);
...
router.post("/admin/upload-soal", isAdmin, uploadExcel.single("fileExcel"), async (req, res) => {
  ...
  try {
    ...
    for (const row of rows) {
      ...
      if (!validPaket.includes(paket)) { skipped++; continue; }

      await db.query(
        `INSERT INTO questions (paket, materi, soal, opsi_a, opsi_b, opsi_c, opsi_d, opsi_e, kunci) 
         VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`,
        [paket, row.materi, row.soal, row.a, row.b, row.c, row.d, row.e, row.kunci]
      );
      inserted++;
    }

    if (fs.existsSync(req.file.path)) fs.unlinkSync(req.file.path);
    res.redirect(`/dashboardAdmin?message=Import+berhasil:+${inserted}+soal+ditambah,+${skipped}+dilewati.`);
  } catch (err) {
...
router.get("/admin/kelola-soal/:paket", isAdmin, async (req, res) => {
  ...
  try {
    ...
    res.render("admin/kelolaSoal", { paket: namaPaket, soalList, config, totalAktif });
  } catch (err) {
    console.error(err);
...
router.post("/admin/toggle-all-soal", isAdmin, async (req, res) => {
  ...
  try {
    await db.query(
      "UPDATE questions SET is_active = ? WHERE paket = ?",
      [is_active, paket]
    );
    res.redirect(`/admin/kelola-soal/${encodeURIComponent(paket)}?message=Semua+soal+berhasil+diperbarui.`);
  } catch (err) {
    console.error(err);
...
// ========================
router.post("/admin/update-jumlah-soal", isAdmin, async (req, res) => {
  const { paket, jumlah_soal } = req.body;
  try {
    await db.query(
      "UPDATE paket_ujian SET jumlah_soal = ? WHERE nama_paket = ?",
      [parseInt(jumlah_soal), paket]
    );
    res.redirect(`/admin/kelola-soal/${encodeURIComponent(paket)}?message=Jumlah+soal+diperbarui.`);
  } catch (err) {
    console.error(err);
    res.redirect(`/admin/kelola-soal/${encodeURIComponent(paket)}?error=Gagal+update.`);
  }
...
router.post("/deleteAccountFromAdmin", isAdmin, async (req, res) => {
  ...
  try {
    const [payments] = await db.query(
      "SELECT bukti_transfer FROM payments WHERE user_id = ?", [id]
    );
    payments.forEach((p) => {
      if (p.bukti_transfer) {
