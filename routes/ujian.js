...
router.post('/mulai', async (req, res) => {
    ...
    try {
        ...
        if (!token) {
            return res.send("Error: Token tidak terkirim dari form!");
        }

        const jamMulai = new Date();
        await db.query(
            'UPDATE users SET status_ujian = "SEDANG_UJIAN", waktu_mulai = ? WHERE id = ?',
            [jamMulai, userId]
        );
...
router.post('/simpan-jawaban', async (req, res) => {
    try {
        ...
        console.log(`Jawaban User ${userId} untuk soal ${questionId} disimpan: ${jawaban}`);
...
router.post('/selesai', async (req, res) => {
    try {
        ...
        await db.query("UPDATE users SET status_ujian = 'SELESAI', skor = ? WHERE id = ?", [skorFinal, userId]);
