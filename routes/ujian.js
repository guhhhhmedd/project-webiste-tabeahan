...
async function getPgConfig(namaPaket) {
  ...
  if (pg.type === "PER_SUBTEST") {
    ...
    pg.perSubtest = {};
    subRows.forEach((s) => {
      pg.perSubtest[s.materi_id] = s.min_skor;
    });
  }
...
router.post("/mulai", isLogin, async (req, res) => {
  const { paket_pilihan, nomor_to, token_input } = req.body;
  const userId = req.session.user.id;

  try {
    const [payment] = await db.query(
      `SELECT id, token_ujian FROM payments
    ...
    if (paymentId) {
      ...
      if (!tokenAsli) {
        return res.send(`<script>
          alert('Token ujian belum tersedia untuk TO ini. Hubungi admin.');
          window.location.href='/dashboardPembayaranUjian';
        </script>`);
      }

      if (!tokenInput || tokenInput !== tokenAsli) {
        return res.send(`<script>
          alert('Token ujian salah atau belum diisi! Periksa kembali token yang diberikan admin.');
          window.location.href='/dashboardPembayaranUjian';
        </script>`);
      }
    }
...
router.get("/soal/:index", isLogin, isSedangUjian, async (req, res) => {
  ...
  try {
    if (
      !sesi.soalIds ||
      sesi.soalIds.length === 0 ||
      currentIndex < 0 ||
      currentIndex >= sesi.soalIds.length
    ) {
      await db.query("UPDATE users SET status_ujian = 'IDLE' WHERE id = ?", [
        userId,
      ]);
    ...
    res.render(
      ...
      (renderErr, html) => {
        if (renderErr) {
          console.error("Error render ujian-soal:", renderErr);
          return res.redirect(
            "/dashboardPembayaranUjian?uploadError=" +
              encodeURIComponent(
                "Gagal menampilkan halaman soal. Ujian dibatalkan, silakan mulai ulang.",
              ),
          );
        }
        res.send(html);
    ...
    res.redirect(
      "/dashboardPembayaranUjian?uploadError=" +
        encodeURIComponent("Gagal memuat soal ujian. Silakan coba lagi."),
    );
  }
});
...
router.get("/selesai-paksa", isLogin, async (req, res) => {
  if (!req.session.ujian) return res.redirect("/dashboardPembayaranUjian");
    ...
    soalRows.forEach((s) => {
      ...
      if (s.tipe_penilaian === "BOBOT_OPSI") {
        const maxBobot = Math.max(
          ...[s.bobot_a, s.bobot_b, s.bobot_c, s.bobot_d, s.bobot_e].map(
            Number,
          ),
        );
        ...
        // BENAR_SALAH
        const bb =
          s.bobot_benar_materi && s.bobot_benar_materi > 0
...
router.get("/review", isLogin, async (req, res) => {
  ...
  try {
    ...
    if (paymentRows.length > 0 && paymentRows[0].created_at) {
      const diff =
        new Date(paymentRows[0].created_at).getTime() +
        7 * 24 * 60 * 60 * 1000 -
        Date.now();
      if (diff > 0) {
    ...
    const totalBenar =
      riwayat[0]?.jml_benar ?? jawabanRows.filter((j) => j.is_benar).length;
