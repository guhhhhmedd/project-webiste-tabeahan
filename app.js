...
const storage = multer.diskStorage({
  ...
  filename: (req, file, cb) => {
    ...
    cb(null, `bukti-${userId}-${uniqueSuffix}${path.extname(file.originalname)}`);
  },
});
...
app.post("/login", async (req, res) => {
  ...
  try {
    ...
    const user = rows[0];
    req.session.user = { id: user.id, username: user.username, role: user.role.toLowerCase() };
...
app.post("/register", async (req, res) => { 
  ...
  try {
    const sql = "INSERT INTO users (username, password, email, role) VALUES (?, ?, ?, 'users')";
    ...
    if (err.code === 'ER_DUP_ENTRY') pesanError = "Username atau Email sudah terdaftar!";
    
    res.render("register", { error: pesanError });
  }
});

app.post("/users/upload-bukti", isLogin, upload.single("bukti"), async (req, res) => {
  if (!req.file) return res.status(400).send("Mohon pilih file.");
  try {
    const userId = req.session.user.id;
    await db.query("INSERT INTO payments (user_id, bukti_transfer, status) VALUES (?, ?, 'PENDING')", [userId, req.file.filename]);
...
app.get("/admin/verify/:id", isLogin, isAdmin, async (req, res) => {
  try {
    ...
    await db.query("UPDATE users SET status = 'LUNAS', token_ujian = ? WHERE id = ?", [tokenBaru, req.params.id]);
    await db.query("UPDATE payments SET status = 'LUNAS' WHERE user_id = ?", [req.params.id]);
