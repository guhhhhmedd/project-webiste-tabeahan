...
async function migrate() {
  try {
    ...
    if (await kolumAda("jawaban_peserta", "paket")) {
      ...
      await db.query(
        `ALTER TABLE jawaban_peserta ADD COLUMN paket VARCHAR(100) DEFAULT NULL AFTER user_id`,
      );
    ...
    if (await kolumAda("jawaban_peserta", "nomor_to")) {
      ...
      await db.query(
        `ALTER TABLE jawaban_peserta ADD COLUMN nomor_to INT DEFAULT 1 AFTER paket`,
      );
